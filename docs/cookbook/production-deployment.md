...
```
Maintenance page [`maintenance.html`](../../app/maintenance.html) will be exported into `/usr/share/nginx/html` from `php-fpm` container later.
...
```
We edit configuration file `/var/lib/pgsql/10/data/postgresql.conf` of postgresql to match application needs based on our [postgres.conf](../../docker/postgres/postgres.conf).
...
```
Then we check whether `mailer_master_email_address` property in [`parameters.yml.dist`](../../app/config/parameters.yml.dist) is set correctly.
...
Now we need to copy [`docker-compose-prod-deploy.yml.dist`](../../docker/conf/docker-compose-prod-deploy.yml.dist) into folder on the production server as `docker-compose.yml`.  
...
With each update of master branch in our repository we need to rebuild image based on [Docker Image Building](./production-deployment.md#docker-image-building) section.
